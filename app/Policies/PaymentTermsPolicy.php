<?php

namespace App\Policies;

use App\Models\PaymentTerms;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class PaymentTermspolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        //
        // if($user->hasRole(['Admin']) ){
        //     return true;
        // }
        // return false;
        if($user->hasPermissionTo('View PaymentTerms')){
            return true;
        }
        return false;
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, PaymentTerms $paymentTerms)
    {
        //
        if($user->hasPermissionTo('View PaymentTerms')){
            return true;
        }
        return false;
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user)
    {
        //
        if($user->hasPermissionTo('Create PaymentTerms')){
            return true;
        }
        return false;
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, PaymentTerms $paymentTerms)
    {
        //
        if($user->hasPermissionTo('Update PaymentTerms')){
            return true;
        }
        return false;
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, PaymentTerms $paymentTerms)
    {
        //
        if($user->hasPermissionTo('Delete PaymentTerms')){
            return true;
        }
        return false;
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, PaymentTerms $paymentTerms)
    {
        //
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, PaymentTerms $paymentTerms)
    {
        //
    }
}
